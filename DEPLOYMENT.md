...
| Region | `us-central1` |
| Service name | `content-management-api` |
...
```bash
docker build -t content-management-api:local .
docker run --rm -p 8080:8080 content-management-api:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/content-management-api:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy content-management-api \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
